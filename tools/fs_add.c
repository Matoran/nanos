...
void find_block_and_write(__uint8_t *block, FILE *img, superblock_t sb, inode_t *inode, int block_number);

__uint32_t find_free_inode(FILE *img, superblock_t superblock);

__uint32_t *read_block(__uint32_t block_number, superblock_t sb, FILE *img);

void goto_block_id(FILE *img, __uint32_t block_id, superblock_t sb);

void write_block(__uint32_t block_number, __uint8_t *block, superblock_t sb, FILE *img);
...
void fs_add(char filename[], char img_name[]){
    ...
    if (img != NULL && file != NULL) {
        ...
        __uint32_t inode_id = find_free_inode(img, sb);
        int offset = inode_id_to_offset(sb, inode_id);
        strcpy(inode.name, filename);
        __uint8_t *block = calloc_error(sb.block_size, sizeof(__uint8_t));
        int result;
...
__uint32_t find_free_block(FILE *img, superblock_t sb) {
...
void find_block_and_write(__uint8_t *block, FILE *img, superblock_t sb, inode_t *inode, int block_number) {
    ...
    if (block_number < sb.direct) {
        __uint32_t block_id = find_free_block(img, sb);
    ...
    if (block_number < sb.single_indirect) {
        ...
        __uint32_t *indirect_block = read_block(inode->blocks[NUMBER_OF_BLOCKS - 2], sb, img);
    ...
    if (block_number < sb.double_indirect) {
        ...
        __uint32_t *indirect_block = read_block(inode->blocks[NUMBER_OF_BLOCKS - 1], sb, img);
        if (indirect_block[block_number / sb.single_indirect] == 0) {
            indirect_block[block_number / sb.single_indirect] = find_free_block(img, sb);
        }
        __uint32_t *double_indirect_block = read_block(indirect_block[block_number / sb.single_indirect], sb, img);
...
__uint32_t *read_block(__uint32_t block_number, superblock_t sb, FILE *img) {
    goto_block_id(img, block_number, sb);
    __uint32_t *block = calloc_error(sb.block_size, 1);
